...
class DbFixture:
    ...
    def get_not_full_group_list(self):
        # returns list of groups which contains not all the contacts in the system
        list = []
        cursor = self.connection.cursor()
        try:
            cursor.execute("select group_id, group_name, group_header, group_footer from group_list gl where exists "
                           "(select null from addressbook where id not in (select id from address_in_groups ag where ag.group_id = gl.group_id))")
            for row in cursor:
                (id, name, header, footer) = row
                list.append(Group(id=str(id), name=name, header=header, footer=footer))
        finally:
    ...
    def get_contact_list_not_in_group(self, groupid):
        list = []
        cursor = self.connection.cursor()
        try:
            cursor.execute("select id, firstname, lastname from addressbook where id not in (select id from address_in_groups where group_id = " + groupid + ") and deprecated = '0000-00-00 00:00:0'")
            for row in cursor:
                (id, firstname, lastname) = row
                list.append(Contact(id=str(id), firstname=firstname, lastname=lastname))
        finally:
